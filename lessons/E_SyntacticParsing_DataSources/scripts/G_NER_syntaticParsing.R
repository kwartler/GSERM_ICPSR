#' Purpose: Apply openNLP, named entity reco
#' Author: Ted Kwartler
...
# Get data & Organize
tmp           <- list.files(path = folderPath, pattern = '.txt', full.names = T)
emails        <- lapply(tmp, readLines)
names(emails) <- gsub('.txt', '', list.files(path = folderPath, pattern = '.txt'))
...
allEmails <- lapply(emails,txtClean)
allEmails[[2]][[1]][1]
...
people       <- lapply(allData, subset, grepl("*person", features))
location     <- lapply(allData, subset, grepl("*location", features))
organization <- lapply(allData, subset, grepl("*organization", features))
