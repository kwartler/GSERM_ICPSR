...
# Read in some data
unicorns <- readRDS('~/Desktop/GSERM_ICPSR/lessons/A_Setup_Intro_Basics/Emoji_example/unicorns.rds')
...
subTxt <- mgsub::mgsub(unicorns$text[1:1000], 
                       emojis$emoji, 
                       paste0(' ', emojis$name,' '))
Sys.time() - st #10x longer for 1000 tweets, imagine more! ~1.5min
subTxt[c(720, 804)] 

# Converts to ASCII so not always helpful, but works in a single line
